...
export const signIn = createAsyncThunk("auth/signIn", async (signInInfo, { rejectWithValue }) => {
    try {
        ...
        if (userRole !== signInInfo.role) {
            Swal.fire({ icon: "error", title: i18next.t("LOGIN_FAIL"), text: "Permission Denied" });
        ...
        Swal.fire({ title: i18next.t("GOOD_JOB"), text: i18next.t("LOGIN_SUCCESS"), icon: "success" });
...
export const logout = createAsyncThunk("auth/logout", async (_, { dispatch }) => {
    ...
    Swal.fire({ title: i18next.t("GOOD_JOB"), text: i18next.t("LOGOUT_SUCCESS"), icon: "success" });
});
