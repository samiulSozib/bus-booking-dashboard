  ...
  async (cityData, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        `${base_url}/admin/location/cities`,
        formData,
        {
          headers: {
            Authorization: `${token}`,
...
export const editCity = createAsyncThunk(
  ...
  async ({ cityId, updatedData }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        `${base_url}/admin/location/cities/update`,
        formData,
        {
          headers: {
            Authorization: `${token}`,
...
const citySlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(showCity.fulfilled, (state, action) => {
        state.selectedCity = action.payload;
      })
      .addCase(showCity.rejected, (state, action) => {
        state.error = action.payload;
      })
      ...
      .addCase(addCity.fulfilled, (state, action) => {
        state.cities.unshift(action.payload); // add to start
        if (state.cities.length > 20) {
          state.cities.pop(); // remove last
        }
