...
const getCitySchema = (t) =>
  Yup.object().shape({
    ...
    cityCode: Yup.string().required(t("city.codeRequired")),
...
export default function CityList() {
  ...
  const { cities, selectedCity, loading, pagination } = useSelector(
    (state) => state.cities
  );
  ...
  const [showModalCountryDropdown, setShowModalCountryDropdown] =
    useState(false);
  const [showModalProvinceDropdown, setShowModalProvinceDropdown] =
    useState(false);
    ...
    if (modalSelectedCountryId) {
      dispatch(
        fetchProvinces({
          ...
          searchTag: modalProvinceSearchTag,
        })
      );
    }
  ...
  useEffect(() => {
    if (selectedCity && isEditMode) {
      setCityName({
        ...
        ps: selectedCity.name.ps,
        fa: selectedCity.name.fa,
      });
    ...
    try {
      ...
      if (isEditMode) {
        const resultAction = await dispatch(
          editCity({ cityId: currentCityId, updatedData: cityData })
        );
        if (editCity.fulfilled.match(resultAction)) {
          Swal.fire({
            icon: "success",
            title: t("success"),
            text: t("cityUpdateSuccess"),
          });
        }
      } else {
        const resultAction = await dispatch(addCity(cityData));
        if (addCity.fulfilled.match(resultAction)) {
          Swal.fire({
            icon: "success",
            title: t("success"),
            text: t("cityAddSuccess"),
          });
        }
      }
      ...
      if (error instanceof Yup.ValidationError) {
        ...
        Swal.fire({
          icon: "error",
          title: t("error"),
          text: error || t("failedToAddEditCity"),
        });
      }
    }
  };
              ...
              <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
                {provinces
                  .filter((province) =>
                    province.name
                      .toLowerCase()
                      .includes(provinceSearchTag.toLowerCase())
                  )
                  .map((province) => (
                    <div
                      key={province.id}
                    ...
                    <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
                      {countries
                        .filter((country) =>
                          country.name
                            .toLowerCase()
                            .includes(modalCountrySearchTag.toLowerCase())
                        )
                        .map((country) => (
                          <div
                            key={country.id}
                      ...
                      <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
                        {provinces
                          .filter((province) =>
                            province.name
                              .toLowerCase()
                              .includes(modalProvinceSearchTag.toLowerCase())
                          )
                          .map((province) => (
                            <div
                              key={province.id}
                              onClick={() =>
                                handleModalProvinceSelect(province)
                              }
                              className="px-4 py-2 cursor-pointer hover:bg-gray-100"
                            >
                              {province.name}
