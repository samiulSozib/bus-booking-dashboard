...
export default function TripCancellationPolicyList() {
  ...
  const { policy, loading, error } = useSelector(
    (state) => state.tripCancellationPolicy
  );
  const { vendorList } = useSelector((state) => state.users);

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isEditMode, setIsEditMode] = useState(false);
  const [penaltySteps, setPenaltySteps] = useState([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        title: t("success"),
        text: isEditMode
          ? t("policyUpdateSuccessfully")
          : t("policyCreatedSuccessfully"),
      });
  ...
  const updatePenaltyStep = (index, field, value) => {
    ...
    const isFirstZeroStep =
      newSteps[index].hours === 0 && index === firstZeroIndex;
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                ...
                {penaltySteps.map((step, index) => {
                  ...
                  const firstZeroIndex = penaltySteps.findIndex(
                    (s) => s.hours === 0
                  );
                  // Check if this is the first 0-hour step
                  const isFirstZeroStep =
                    step.hours === 0 && index === firstZeroIndex;
                  ...
                  return (
                    <div key={index} className="flex gap-2 mb-3 items-end">
                      ...
                      {!(
                        isFirstZeroStep &&
                        penaltySteps.filter((s) => s.hours === 0).length === 1
                      ) && (
                        <button
                          type="button"
                ...
                {policy.penalty_steps
                  ...
                  .map((step, index) => (
                    <TableRow key={`zero-${index}`}>
                      ...
                      <TableCell className="py-3">{step.percentage}%</TableCell>
                    </TableRow>
                  ))}
